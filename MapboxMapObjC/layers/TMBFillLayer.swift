...
extension FillLayer {
    public func mapTo(_ layer:inout TMBFillLayer) {
        ...
        layer.fillSortKey = TMBValue.fromSwiftValue(self.fillSortKey)
        layer.fillAntialias = TMBValue.fromSwiftValue(self.fillAntialias)
        layer.fillColor = TMBValue.fromSwiftValue(self.fillColor)
        layer.fillColorTransition = self.fillColorTransition?.objcValue()
        layer.fillOpacity = TMBValue.fromSwiftValue(self.fillOpacity)
        layer.fillOpacityTransition = self.fillOpacityTransition?.objcValue()
        layer.fillOutlineColor = TMBValue.fromSwiftValue(self.fillOutlineColor)
        layer.fillOutlineColorTransition = self.fillOutlineColorTransition?.objcValue()
        layer.fillPattern = TMBValue.fromSwiftValue(self.fillPattern)
        layer.fillPatternTransition = self.fillPatternTransition?.objcValue()
        layer.fillTranslate = TMBValue.fromSwiftValue(self.fillTranslate)
        layer.fillTranslateTransition = self.fillTranslateTransition?.objcValue()
        layer.fillTranslateAnchor = TMBValue.fromSwiftValue(self.fillTranslateAnchor)
    }
}
