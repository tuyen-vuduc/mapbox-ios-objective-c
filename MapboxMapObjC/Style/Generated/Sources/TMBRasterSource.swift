...
@objc open class TMBRasterSource: NSObject, TMBSource {
    @objc public init(id: String) {
        self.id = id
        self.type = TMBSourceType.raster
    }
...
extension TMBRasterSource {
    func mapTo(_ source: inout RasterSource) {
        source.url = self.url
...
extension RasterSource {
    func mapTo(_ source:inout TMBRasterSource) {
        source.url = self.url
        source.tiles = self.tiles
        source.bounds = self.bounds
        source.minzoom = self.minzoom?.double()
...
extension TMBRasterSource: SwiftValueConvertible {
    public func unwrap() -> RasterSource {
...
extension RasterSource {
    func wrap() ->  TMBRasterSource {
        var result = TMBRasterSource(id: self.id)
        self.mapTo(&result)
        return result
    }
}