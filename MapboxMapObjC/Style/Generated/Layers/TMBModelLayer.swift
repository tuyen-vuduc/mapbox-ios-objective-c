...
extension TMBModelLayer {
    ...
    func mapTo(_ dest: inout ModelLayer) {
        ...
        dest.modelId = self.modelId?.string()
        dest.modelAmbientOcclusionIntensity = self.modelAmbientOcclusionIntensity?.double()
        dest.modelAmbientOcclusionIntensityTransition = self.modelAmbientOcclusionIntensityTransition?.unwrap()
        dest.modelCastShadows = self.modelCastShadows?.bool()
        dest.modelColor = self.modelColor?.styleColor()
        dest.modelColorTransition = self.modelColorTransition?.unwrap()
        dest.modelColorMixIntensity = self.modelColorMixIntensity?.double()
        dest.modelColorMixIntensityTransition = self.modelColorMixIntensityTransition?.unwrap()
        dest.modelCutoffFadeRange = self.modelCutoffFadeRange?.double()
        dest.modelEmissiveStrength = self.modelEmissiveStrength?.double()
        dest.modelEmissiveStrengthTransition = self.modelEmissiveStrengthTransition?.unwrap()
        dest.modelHeightBasedEmissiveStrengthMultiplier = self.modelHeightBasedEmissiveStrengthMultiplier?.arrayOfDouble()
        ...
        dest.modelOpacity = self.modelOpacity?.double()
        dest.modelOpacityTransition = self.modelOpacityTransition?.unwrap()
        dest.modelReceiveShadows = self.modelReceiveShadows?.bool()
        dest.modelRotation = self.modelRotation?.arrayOfDouble()
        dest.modelRotationTransition = self.modelRotationTransition?.unwrap()
        dest.modelRoughness = self.modelRoughness?.double()
        dest.modelRoughnessTransition = self.modelRoughnessTransition?.unwrap()
        dest.modelScale = self.modelScale?.arrayOfDouble()
        dest.modelScaleTransition = self.modelScaleTransition?.unwrap()
        dest.modelScaleMode = self.modelScaleMode?.modelScaleMode()
        dest.modelTranslation = self.modelTranslation?.arrayOfDouble()
        dest.modelTranslationTransition = self.modelTranslationTransition?.unwrap()
        dest.modelType = self.modelType?.modelType()
    }
}
