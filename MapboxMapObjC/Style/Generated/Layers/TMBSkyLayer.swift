...
extension TMBSkyLayer: SwiftValueConvertible {
    ...
    func mapTo(_ dest: inout SkyLayer) {
        ...
        dest.skyAtmosphereColor = self.skyAtmosphereColor?.styleColor()
        dest.skyAtmosphereHaloColor = self.skyAtmosphereHaloColor?.styleColor()
        dest.skyAtmosphereSun = self.skyAtmosphereSun?.arrayOfDouble()
        dest.skyAtmosphereSunIntensity = self.skyAtmosphereSunIntensity?.double()
        dest.skyGradient = self.skyGradient?.styleColor()
        dest.skyGradientCenter = self.skyGradientCenter?.arrayOfDouble()
        dest.skyGradientRadius = self.skyGradientRadius?.double()
        dest.skyOpacity = self.skyOpacity?.double()
        dest.skyOpacityTransition = self.skyOpacityTransition?.unwrap()
        dest.skyType = self.skyType?.skyType()
    }
}
extension SkyLayer {
