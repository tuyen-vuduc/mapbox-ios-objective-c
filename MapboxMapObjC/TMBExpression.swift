...
@objc open class TMBExpression: NSObject {
    ...
    public var `operator`: TMBOperator? {
        return tmbOperatorMapping[_self.operator]
    }
    
    /// Length of time before a transition begins in seconds.
    public var arguments: [Any] {
        return _self.arguments.map({
            switch $0 {
            case let expression as Expression:
                return TMBExpression(expression)
            default:
                return $0
            }
        })
    ...
    @objc class public func create(withOperator `operator`: TMBOperator) -> TMBExpression{
        return TMBExpression(Expression(operatorMapping[`operator`]!))
    }
    
    @objc class public func create(withOperator `operator`: TMBOperator, arguments: [Any]) -> TMBExpression{
        return TMBExpression(Expression(operator: operatorMapping[`operator`]!, arguments: getSwiftArguments(arguments)))
    }
...
let tmbOperatorMapping: [Expression.Operator:TMBOperator] = [
    /// For two inputs, returns the result of subtracting the second input from the first. For a single input, returns the result of subtracting it from 0.
    .subtract: .subtract,
